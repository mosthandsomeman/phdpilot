...
export default function LoginPage() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? String(err.message) : t("auth.loginFailed"));
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="relative flex min-h-screen items-center justify-center mesh-bg px-6">
      ...
      <Card className="w-full max-w-md">
        ...
        <p className="mt-6 text-center text-sm text-muted">
          {t("auth.noAccount")}{" "}
          <Link href="/register" className="text-violet-600 hover:text-violet-500 dark:text-violet-400">
            {t("auth.registerFree")}
          </Link>
        </p>
      </Card>
